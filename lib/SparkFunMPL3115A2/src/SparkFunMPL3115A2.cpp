/*
 ...
 .enableEventFlags() Sets the fundamental event flags. Required during setup.
  ...
  Updated by JonathanWB: June 22nd, 2020
  - Modified to work with i2c_t3 library instead of Wire for teensy device
...
void MPL3115A2::begin(i2c_t3 &wirePort, uint8_t deviceAddress)
{
...
float MPL3115A2::readAltitude()
{
	...
	if (_i2cPort->requestFrom((int) _I2Caddress, (int) 3) != 3) { // Request three bytes
		return -999;
	}
...
float MPL3115A2::readTemp()
{
	...
	_i2cPort->endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
	if (_i2cPort->requestFrom((int) _I2Caddress, (int) 2) != 2) { // Request two bytes
		return -999;
	}
...
byte MPL3115A2::IIC_Read(byte regAddr)
{
  ...
  _i2cPort->endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
  _i2cPort->requestFrom((int) _I2Caddress, (int) 1); // Request the data...
